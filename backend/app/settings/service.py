...
class SystemSettingsDraftService:
    ...
    async def read(self) -> dict:
        ...
        active_draft = await self.session.get(SystemSettingsDraft, state.active_version) if state and state.active_version else None
        return self._project(draft=draft, active_draft=active_draft, state=state)
    ...
    async def save(self, *, actor: str, payload: object) -> dict:
        ...
        active_draft = await self.session.get(SystemSettingsDraft, state.active_version) if state.active_version else None
        return self._project(draft=draft, active_draft=active_draft, state=state)
    ...
    async def begin_application(self, *, actor: str, version: object) -> dict:
        ...
        self._validate_saved_version(draft, provider_api_key=provider_api_key)
        try:
            await self.runtime.validate(settings=draft.settings, provider_api_key=provider_api_key)
    ...
    def _validate_saved_version(self, draft: SystemSettingsDraft, *, provider_api_key: str | None) -> None:
        payload = {**draft.settings, "provider_api_key": provider_api_key}
    ...
    def _validate(self, payload: object) -> tuple[dict, str | None]:
        ...
        provider_type = str(raw["provider_type"]).strip().lower()
        if provider_type not in _SUPPORTED_PROVIDERS:
            fields["provider_type"] = "unsupported generation provider"

        model = str(raw["model"]).strip()
        if not _IDENTIFIER_PATTERN.fullmatch(model):
            fields["model"] = "model identifier is unsafe or unsupported"

        service_url = str(raw["service_url"]).strip()
        if not self._is_safe_service_url(service_url):
            fields["service_url"] = "must be an http or https URI without credentials"
    ...
    def _is_safe_service_url(value: str) -> bool:
        try:
            ...
            return False
        return bool(value and parsed.scheme in {"http", "https"} and parsed.hostname and not parsed.username and not parsed.password)
    ...
    def _project(
        *,
        draft: SystemSettingsDraft | None,
        active_draft: SystemSettingsDraft | None,
        state: SystemSettingsState | None,
    ) -> dict:
        application = None
        ...
        return {
            "draft": {
                **{key: draft.settings.get(key, default) for key, default in _DEFAULT_DRAFT.items()},
            ...
            "active": (
                {
                    **{key: active_draft.settings.get(key, default) for key, default in _DEFAULT_DRAFT.items()},
                    "provider_api_key": {"configured": bool(active_draft.sealed_secrets.get("provider_api_key"))},
