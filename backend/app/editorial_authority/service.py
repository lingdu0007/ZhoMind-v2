...
class EditorialAuthorityService:
    ...
    async def get_retrieval_authority(self, entry_id: str, *, now: datetime | None = None) -> dict[str, Any]:
        ...
        release_assurance_snapshot = await self._release_assurance_snapshot(draft)
        authority.update(self._retrieval_content_projection(draft, sources, release_assurance_snapshot))
        return authority

    @staticmethod
    def _retrieval_content_projection(
        draft: CreateEditorialEntryRequest,
        sources: list[dict[str, Any]],
        release_assurance_snapshot: dict[str, Any] | None,
    ) -> dict[str, Any]:
    ...
    async def get_retrieval_authority_for_revision(
        ...
        *,
        ...
        release_assurance_snapshot = await self._release_assurance_snapshot(draft)
        authority.update(self._retrieval_content_projection(draft, sources, release_assurance_snapshot))
        authority["source_revalidated_at"] = now.isoformat() if now is not None else None
        return authority
